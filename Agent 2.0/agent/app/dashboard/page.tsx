    ...
    <div className="relative min-h-screen bg-background flex text-zinc-350 font-sans selection:bg-indigo-500/30 selection:text-white overflow-hidden">
      ...
      {isSettingsOpen && selectedPlatform && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="glassmorphism-card rounded-2xl max-w-2xl w-full border border-zinc-850 shadow-2xl p-6 relative flex flex-col max-h-[85vh] overflow-hidden text-zinc-350">
            {/* Header */}
            <div className="flex items-center justify-between pb-4 border-b border-zinc-900">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-neutral-900/60 border border-zinc-850 rounded-xl">
                  <img src={selectedPlatform.logoSrc} className="h-8 w-8" alt={selectedPlatform.name} />
                </div>
                <div>
                  <h3 className="text-sm font-bold text-white uppercase tracking-wider">{selectedPlatform.name} MCP Settings</h3>
                  <span className="text-[10px] text-zinc-500">Model Context Protocol Configurations</span>
                </div>
              </div>
              <button
                onClick={() => {
                ...
                className="text-zinc-400 hover:text-white transition-colors text-xs font-bold px-3 py-1.5 rounded-xl border border-zinc-800 bg-neutral-900/60 cursor-pointer"
              >
                Close Dialog
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto py-5 space-y-6">
              ...
              {/* Description */}
              <div>
                <h4 className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest mb-1.5">Platform Capabilities</h4>
                <p className="text-xs text-zinc-400 leading-relaxed font-sans">{selectedPlatform.description}</p>
              </div>

              {/* MCP Tools list */}
              <div>
                <h4 className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest mb-3">Available MCP Tools & Actions</h4>
                <div className="space-y-3">
                  {selectedPlatform.mcpTools.map((tool: any, idx: number) => (
                    <div key={idx} className="p-3 bg-neutral-950 rounded-xl border border-zinc-900 flex flex-col gap-1.5">
                      <div className="flex justify-between items-center">
                      ...
                      <p className="text-[11px] text-zinc-500 font-sans">{tool.description}</p>
                      <div className="text-[10px] text-zinc-650 flex gap-1 font-mono">
                        <span className="text-zinc-650 font-bold">Parameters:</span>
                        <span>{tool.params || "None"}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* Gmail Specific Interactive Sandbox runner */}
              {selectedPlatform.id === "gmail" && (
                <div className="pt-4 border-t border-zinc-900 flex flex-col gap-3">
                  {/* Connected Google Account Scope Banner */}
                  <div className="p-3 rounded-xl bg-indigo-950/20 border border-indigo-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                      <span className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
                      <span className="text-[11px] font-bold text-white">Target Account:</span>
                      <span className="text-[11px] font-mono text-indigo-400 font-bold">{user?.email || user?.profile?.name || "bnk.avik@gmail.com"}</span>
                    </div>
                    <button
                      onClick={handleGoogleOAuthConnect}
                      className="text-[10px] font-bold px-3 py-1.5 bg-gradient-to-r from-red-600 to-amber-600 hover:from-red-500 hover:to-amber-500 text-white rounded-lg transition-all cursor-pointer shadow flex items-center justify-center gap-1.5 shrink-0"
                    >
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-1">
                    <div>
                      <h4 className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest">Gmail MCP Live Sandbox</h4>
                      <span className="text-[9px] text-zinc-500 block mt-0.5 font-sans">Query inbox messages using simulated gmail_list_messages endpoint</span>
                    </div>
                    
                    <div className="flex items-center gap-3 self-end sm:self-auto">
                      <div className="flex items-center gap-1.5">
                        <label className="text-[10px] text-zinc-550 font-mono">Limit:</label>
                        <select
                          value={emailLimit}
                          onChange={(e) => setEmailLimit(Number(e.target.value))}
                          className="bg-neutral-900 border border-zinc-850 rounded-lg px-2 py-1 text-[10px] font-mono text-white outline-none"
                        >
                      ...
                      <button
                        ...
                        className="text-[10px] font-bold py-1.5 px-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg transition-colors cursor-pointer disabled:opacity-40"
                      >
                        {fetchingEmails ? "Running Tool..." : "Run gmail_list_messages"}
                      </button>
                    </div>
                  </div>

                  {gmailEmails.length > 0 ? (
                    <div className="rounded-xl border border-zinc-900 bg-neutral-950 overflow-hidden font-sans text-xs">
                      <div className="p-3 bg-zinc-900/60 font-bold border-b border-zinc-900 grid grid-cols-12 gap-2 text-zinc-400 text-[10px] uppercase tracking-wider">
                      ...
                      <div className="divide-y divide-zinc-900">
                        {gmailEmails.map((email) => (
                          <div key={email.id} className="p-3 grid grid-cols-12 gap-2 items-start hover:bg-neutral-900/40 transition-colors">
                            <div className="col-span-4 text-zinc-300 font-medium truncate">{email.from}</div>
                            <div className="col-span-5">
                              <span className="text-white font-semibold block truncate">{email.subject}</span>
                              <span className="text-zinc-500 text-[10px] block mt-0.5 line-clamp-1">{email.body}</span>
                            </div>
                            <div className="col-span-3 text-right text-[10px] text-zinc-500 pt-0.5">
                    ...
                    <div className="p-6 bg-neutral-950 border border-zinc-900 rounded-xl text-center text-zinc-650 text-xs font-sans">
                      {fetchingEmails ? "Fetching inbox records from Gmail MCP..." : "Inbox sandbox not run yet. Click 'Run gmail_list_messages' to query via MCP."}
                    </div>
                  )}
                </div>
              )}
